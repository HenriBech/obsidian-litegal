			...
			async (source, el, ctx) => {
				...
				const image_list = source
					...
					.map((image) => {
						...
						if (image_file == null) {
							new Notice(
								`LiteGallery: Image not found: ${image}`
							);
							return null;
						}
				...
				if (image_list.length > 0) {
					...
					const active_image_container_inner =
						active_image_container.createEl("div", {
							cls: "litegal-active-inner",
						});

					// Create the active image element and set its source to the first image in the list
					const active_image =
						active_image_container_inner.createEl("img");
					...
					larrow.onclick = () => {
						active_slide =
							(active_slide - 1 + image_list.length) %
							image_list.length;
						active_image.src = image_list[active_slide];
					};
					...
					const preview_outer_container = gallery.createEl("div", {
						cls: "litegal-preview-outer",
					});

					// Create the left arrow element for preview scrolling and handle mouse events to control scroll speed
					const preview_larrow = preview_outer_container.createEl(
						"div",
						{
							text: "<",
							cls: "litegal-arrow litegal-arrow-left",
					...
					const preview_rarrow = preview_outer_container.createEl(
						"div",
						{
							text: ">",
							cls: "litegal-arrow litegal-arrow-right",
					...
					const preview_container = preview_outer_container.createEl(
						"div",
						{
							cls: "litegal-preview",
						}
